package com.example.sarkarisnap.core.utils
...
fun shareViaX(context: Context, text: String, url: String) {
    ...
    try {
        ...
        val browserIntent = Intent(Intent.ACTION_VIEW, Uri.parse("https://twitter.com/intent/tweet?text=${Uri.encode(text)}&url=${Uri.encode(url)}"))
        try {
            context.startActivity(browserIntent)
        } catch (e2: Exception) {
            Toast.makeText(context, "X (Twitter) not installed and browser could not open.", Toast.LENGTH_SHORT).show()
            shareViaMore(context, text, url) // Fallback to generic share
        }
    }
}
...
fun shareViaFacebook(context: Context, text: String, url: String) {
    ...
    try {
        ...
        val browserIntent = Intent(Intent.ACTION_VIEW, Uri.parse("https://www.facebook.com/sharer/sharer.php?u=${Uri.encode(url)}&quote=${Uri.encode(text)}"))
        try {
            context.startActivity(browserIntent)
        } catch (e2: Exception) {
            Toast.makeText(context, "Facebook not installed and browser could not open.", Toast.LENGTH_SHORT).show()
            shareViaMore(context, text, url) // Fallback to generic share
        }
    }
}
